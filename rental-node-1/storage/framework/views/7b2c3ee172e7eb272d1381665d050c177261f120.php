...
$('#itemTable').DataTable( {
    ...
    "serverSide": true,
    "ajax": {
        "url": <?php echo json_encode(action("BookingController@newItem")); ?> + "/new",
        "type": "POST"
    },
    "columns": [
      {"data": "id"},
      ...
      {"data": "price"},
      {
        ...
        "render": function(data) {
          return datatableDetail(<?php echo json_encode(action("BookingController@newItem")); ?> + "/" + data.kode_booking.replace("#", "") + "/detail") +
                 datatableConfirm(<?php echo json_encode(action("BookingController@newItem")); ?> + "/" + data.id + "/confirm") +
                 datatableReject(<?php echo json_encode(action("BookingController@newItem")); ?> + "/" + data.id + "/reject")
        }
      }
    ],
    "columnDefs": [
