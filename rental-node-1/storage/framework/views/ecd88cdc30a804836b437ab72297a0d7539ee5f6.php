...
$('#itemTable').DataTable( {
    ...
    "serverSide": true,
    "ajax": {
        "url": <?php echo json_encode(action("BookingController@newItem")); ?> + "/confirmed",
        "type": "POST"
    },
    "columns": [
      {"data": "id"},
      ...
      {"data": "price"},
      {
        ...
        "render": function(data) {
          return datatableDetail(<?php echo json_encode(action("BookingController@newItem")); ?> + "/" + data.id +        "/update") +
                 datatableCancel(<?php echo json_encode(action("BookingController@newItem")); ?> + "/" + data.id + "/cancel")
        }
      }
    ]
} );
